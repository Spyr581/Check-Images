...
REM В итоге получается таблица, где будет указан максимально возможный порог каждого шаблонного изображения
REM на данном скриншоте с заданной точностью (precision). Также будут указаны координаты левой верхней точки, где
REM шаблон был найден на скриншоте.
...
REM 	Диапазон порога устанавливается параметрами min и max. Параметр HOME_DIR - это папка, где лежит
...
REM -x, --max       - maximum threshold value in range, default=0.999999, optional parameter
REM -p, --precision - the precision with which templates will be searched in the screen image, default=0.001, optional parameter

python "%HOME_DIR%\check_images.py" -t "%TEMPLATES%" -i "%SCREENSHOT%" -p 0.0001
